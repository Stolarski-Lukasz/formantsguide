...
class BatchAcousticDataProcessor:
    ...
    def get_average_bin_values(self, folder_name, cardinal_vowel, window_length="broadband", output_data_type="spectrogram"):
        """
...
class ParselmouthSpectrogramGrapher(SpectrogramGrapher):
    ...
    def draw_average_spectrogram(self, Y,
        ...
        if title:
            plt.title("CV " + str(cardinal_vowel) + " average " + str(
                window_length) + " spectrogram " + "(n = " + str(sample_size) + ")")
    ...
    # def draw_average_spectrogram(self,
    #                                 colour_scheme="Greys",
    #                                 duration=1,
    #                                 dynamic_range=50,
    #                                 db_coeff=10,
    ...
    #         plt.pcolormesh(X, self.Y, sg_db, vmin=sg_db.max() -
    #                     dynamic_range, cmap=colour_scheme)
    #         if formants:
    ...
    #         if title:
    #             plt.title("CV " + str(self.cardinal_vowel) + " average " + str(
    #                 self.window_length) + " spectrogram " + "(n = " + str(self.sample_size) + ")")
...
class ParselmouthSpectrumGrapher():
    ...
    def get_average_spectrum_slice_values(self, folder_name, cardinal_vowel, window_length="broadband"):
        ...
        for folder in folders_list:
            ...
            for file in files_list:
                if file[folder_length:folder_length+2] == cardinal_vowel:
                    ...
                    spectrogram = get_spectrogram_object(sound_file=file,
                                                         window_length=window_length)
                    spectrum_slice_df = get_spectrum_slice_df(
        ...
        average_spectrum_slice_values = np.mean(
            np.array(singlevowel_powers), axis=0)
    ...
    def draw_average_spectrum(self, Y,
        ...
        if title:
            plt.title("CV " + str(cardinal_vowel) + " average " +
                      window_length + " spectrum " + "(n = " + str(sample_size) + ")")
        ...
        if save:
            if save_name:
                plt.savefig(save_folder + "CV_" + save_name + "_" +
                            window_length + "_average_spectrum", dpi=200)
            else:
    ...
    #     if title:
    #         plt.title("CV " + str(self.cardinal_vowel) + " average " +
    #                   self.window_length + " spectrum " + "(n = " + str(self.sample_size) + ")")
    ...
    #         if save_name:
    #             plt.savefig(save_folder + "CV_" + save_name + "_" +
    #                         self.window_length + "_average_spectrum", dpi=200)
    #         else:
